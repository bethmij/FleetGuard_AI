...
export function VehicleSelection() {
  ...
  useEffect(() => {
    vehicleService
      .getAvailable()
      .then((data) => {
        const mapped: VehicleCard[] = (data.vehicles || []).map((v: { id: number; number_plate: string; make: string; model: string; year: number; health_score: number; photo_url?: string | null; color?: string | null; vehicle_type?: string }) => ({
          id: String(v.id),
          ...
          status: 'Available',
          image: v.photo_url ? `${(import.meta.env.VITE_API_URL || 'http://localhost:3001/api').replace(/\/api\/?$/, '')}${v.photo_url.startsWith('/') ? '' : '/'}${v.photo_url}` : 'https://images.unsplash.com/photo-1449965408869-eaa3f722e40d?w=800',
          color: v.color || 'N/A',
          lastInspection: '—',
  ...
  const handleVehicleSelect = (vehicle: VehicleCard) => {
    setVehicle(vehicle.id, vehicle.number, vehicle.make, vehicle.model, vehicle.year, vehicle.vehicleType);
