...
import { getPhotoTemplate } from '@/constants/photoTemplates';
...
export function PhotoCapture() {
  ...
  const capturePhoto = () => {
    ...
    canvas.toBlob((blob) => {
      if (blob) {
        ...
        const template = getPhotoTemplate(inspection.vehicleType);
        if (currentPhotoIndex < template.length - 1) {
  ...
  const photoTemplate = getPhotoTemplate(inspection.vehicleType);
  const currentPhoto = photoTemplate[currentPhotoIndex] || photoTemplate[0];
  const capturedCount = inspection.photos.filter((p) => p.captured).length;
