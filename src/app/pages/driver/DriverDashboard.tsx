...
export function DriverDashboard() {
  ...
  const [recentInspections, setRecentInspections] = useState<Array<{ id: number; number_plate: string; make: string; model: string; health_score?: number; created_at: string; vehicle_photo_url?: string }>>([]);
  ...
  useEffect(() => {
    inspectionService.getMine(1).then((d: any) => setRecentInspections((d.inspections || []).slice(0, 5))).catch(() => []);
  }, []);

  // Sprint 5: Auto-capture GPS when driver opens the app — runs once on mount
  useEffect(() => {
    gpsService.captureAndSend().then((result) => {
      if (result.success) {
  ...
  const handleLocationUpdate = () => {
    ...
    gpsService.captureAndSend().then((result) => {
      if (result.success) {
        ...
        toast.error(result.reason || 'Could not update location');
      }
              ...
              <div className="flex-1 pb-2">
                <div className="flex items-center gap-2 mb-3">
                  <span className="px-3 py-1.5 rounded-lg text-xs font-medium bg-white/10 backdrop-blur-md border border-white/20 text-gray-900 dark:text-white">
                    DRV-{String(user?.id || 0).padStart(4, '0')}
                  </span>
                </div>
          ...
          <div className="space-y-3">
            {recentInspections.length > 0 ? (
              recentInspections.map((insp, idx) => (
                <InspectionGlassCard
                  key={insp.id}
                  vehicle={insp.number_plate}
                  model={`${insp.make} ${insp.model}`}
                  date={timeAgo(insp.created_at)}
                  score={insp.health_score ?? 0}
                  image={insp.vehicle_photo_url ? `${(import.meta.env.VITE_API_URL || 'http://localhost:3001/api').replace(/\/api\/?$/, '')}${insp.vehicle_photo_url.startsWith('/') ? '' : '/'}${insp.vehicle_photo_url}` : 'https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?w=400'}
                  delay={idx * 50}
                  onClick={() => navigate(`/driver/history/${insp.id}`)}
                />
              ))
            ) : (
...
function InspectionGlassCard({ vehicle, model, date, score, image, delay, onClick }: any) {
  return (
    <div 
      onClick={onClick}
