...
// Map Python severity (minor/moderate/major) → DB (low/medium/high)
const SEVERITY_TO_DB = { minor: 'low', moderate: 'medium', major: 'high' };

// Map Python photo_label → backend photo_type
const LABEL_TO_PHOTO_TYPE = {
  'Front View': 'front',
  'Rear View': 'rear',
  'Left Side': 'left',
  'Right Side': 'right',
...
exports.getMine = async (req, res, next) => {
  try {
    ...
    const r = await pool.query(
      `SELECT i.*, v.number_plate, v.make, v.model, v.health_score, v.photo_url as vehicle_photo_url,
              (SELECT COUNT(*)::int FROM damage_detections WHERE inspection_id=i.id) as damage_count
...
// ─── TRIGGER AI (calls Python FleetGuard AI service) ───────────────────
exports.triggerAI = async (req, res, next) => {
  try {
    ...
    // Sort photos in AI-expected order (Front View, Rear View, ...)
    const photoByType = {};
    photosResult.rows.forEach((p) => { photoByType[p.photo_type] = p; });
    const sortedPhotos = PHOTO_ORDER.map((t) => photoByType[t]).filter(Boolean);
    if (sortedPhotos.length < 8) {
      return res.status(400).json({ error: 'Missing required photo types for AI analysis' });
    }

    const uploadDir = path.resolve(process.env.UPLOAD_DIR || './uploads');
    const form = new FormData();
    form.append('inspection_id', id);

    const inspRow = await pool.query('SELECT vehicle_id FROM inspections WHERE id = $1', [id]);
    const vehicleId = inspRow.rows[0]?.vehicle_id || 'unknown';
    form.append('vehicle_id', String(vehicleId));

    for (const photo of sortedPhotos) {
      const filename = (photo.file_url || '').replace(/^\/uploads\//, '') || path.basename(photo.file_url);
      const filePath = path.join(uploadDir, filename);
      if (!fs.existsSync(filePath)) {
        return res.status(400).json({ error: `Photo file not found: ${photo.photo_type}` });
      }
      form.append('images', fs.createReadStream(filePath), { filename: photo.file_name || filename });
    }

    const aiRes = await axios.post(`${aiUrl}/api/detect`, form, {
    ...
    if (!aiRes.data?.success) {
      return res.status(502).json({
        ...
        detail: aiRes.data?.error || 'Unknown error',
      });
    }

    const { health_score, damages: aiDamages, annotated_images } = aiRes.data;

    // Map photo_label → inspection_photo id for linking
    const photosByLabel = {};
    sortedPhotos.forEach((p, idx) => {
      const label = Object.keys(LABEL_TO_PHOTO_TYPE).find((k) => LABEL_TO_PHOTO_TYPE[k] === p.photo_type);
      if (label) photosByLabel[label] = p;
    });

    // Clear existing damages and insert AI results
    await pool.query('DELETE FROM damage_detections WHERE inspection_id = $1', [id]);

    const dbDamages = [];
    for (const d of aiDamages || []) {
      const photoLabel = d.photo_label || d.photoLabel;
      const photo = photosByLabel[photoLabel] || sortedPhotos[d.photo_index];
      const photoId = photo?.id || null;
      const severityDb = SEVERITY_TO_DB[d.severity] || 'low';
      const bboxJson = {
        location: d.location_description || d.location,
      ...
      const ins = await pool.query(
        `INSERT INTO damage_detections
           (inspection_id, photo_id, damage_type, severity, confidence, bbox_json)
         VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
        [id, photoId, d.damage_type || d.type, severityDb, d.confidence ? Number(d.confidence) * 100 : null, JSON.stringify(bboxJson)]
      );
      dbDamages.push({
        type: d.damage_type || d.type,
        severity: d.severity,
        confidence: d.confidence ? Number(d.confidence) * 100 : 0,
        location: d.location_description || d.location || 'Unknown',
        photo_type: photo?.photo_type,
      });
    }
    ...
    if (inspRow.rows.length) {
      await pool.query(
        ...
        [Math.max(0, Math.min(100, health_score ?? 100)), inspRow.rows[0].vehicle_id]
      );
    }
    ...
    res.json({
      ...
      damage_count: dbDamages.length,
      annotated_images: annotated_images || [],
      analysis_source: aiRes.data?.model_info?.stub_mode ? 'stub' : 'yolov8',
    });
  } catch (err) {
...
exports.reviewInspection = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { review_status, notes, manager_notes } = req.body;
    const notesVal = manager_notes ?? notes ?? null;
    ...
    await pool.query(
      ...
      [id, req.user.id, review_status, notesVal]
    );

    await pool.query(
      `UPDATE inspections SET status = 'reviewed', updated_at = NOW(),
       manager_notes = $1, manager_id = $2, reviewed_at = NOW(), review_status = $3
       WHERE id = $4`,
      [notesVal, req.user.id, review_status, id]
    );
